...
\begin{practiceprob}[Statements and Their Negations]\label{rainstatements}
    Fill in the negation for the statements: \(R\equiv\) ``It is raining.''; \(G\equiv\) ``The grass is wet.''; and \(U\equiv\) ``I have an umbrella.''
        \begin{itemize}
            \item \(\sim R\equiv\) \vspace{2\baselineskip}
            \item \(\sim G\equiv\) \vspace{2\baselineskip}
            \item \(\sim U\equiv\) \vspace{2\baselineskip}
        \end{itemize}
\end{practiceprob}
...
\begin{practiceprob}[Visualizing Truth Values]\label{mainimages}
    ...
    \begin{center}
        ...
        \includegraphics[scale=0.25,alt={Eight images with different configurations of a stick figure with out without rain or an umbrella or wet grass.},width=0.85\linewidth]{images/created/grass_zombies.png}
    \end{center}
\end{practiceprob}
...
\begin{practiceprob}[Relating Statements to Sets]
    For each region of the \emph{Venn Diagram}, write down which picture from practice problem \ref{mainimages} belongs in that region.
    \begin{center}
...
\begin{practiceprob}[True or False]\label{pracprob:tautologies}
...
\begin{exposition}{Some ``Basic'' Logical Rules}\label{expo:logicrules}
...
\begin{practiceprob}
    Which statements in practice problem \ref{pracprob:tautologies} represent which laws in exposition \ref{expo:logicrules}?
