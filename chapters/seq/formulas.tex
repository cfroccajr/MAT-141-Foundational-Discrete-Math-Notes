...
\begin{itemize}
    ...
    \item Consecutive evens:
        \begin{equation}\sum_{i=1}^n 2i=n(n+1)\label{eq:even}\end{equation}
    \item Geometric Sum:
...
\begin{exposition}[Iteration Algorithm]\index{iteration algorithm}\label{exp:iteration}
    Given \(a_0=b\) and an expression \(a_k=c\cdot a_{k-1}+d\):
    \begin{enumerate}
        \item While \(k>0\):
        \begin{enumerate}
        ...
        \item Simplify the final expression with appropriate formula
        \item Generalize
    \end{enumerate}
